...
void main() {
    ...
    vec2 world = vec2(local.x * max(aspect, 0.25), -local.y);
    ...
    float cloud = n1 * 0.55 + n2 * 0.45;
    // Green/teal volume clouds in the fluid.
    vec3 green = vec3(0.04, 0.14, 0.11);
    col += green * cloud * 0.55;
    col += vec3(0.04, 0.12, 0.13) * cloud * (1.0 - depth) * 0.35;
    ...
    col += vec3(0.7, 0.92, 0.98) * bub * tw * 0.04 * (1.0 - rim);
    ...
    if (light_rad > 0.001) {
        ...
        col += vec3(0.65, 0.95, 1.0) * bub * tw * floor_l * 0.55 * (1.0 - rim);
